...
// all valid flags
const possible_flags = ["title", "desc", "status", "priority", "type", "assignee"]
...
export function parse(argv){
    ...
    while (i < args.length){
        ...
        if (current.startsWith("-")){
            ...
            if (!/^--[a-z]/.test(current)) throw new Error(`Invalid flag '${current}': flags must start with --`)
            ...
            if (!possible_flags.includes(flag)) throw new Error(`Unknown flag '${flag}': valid flags are\n${possible_flags.join(', ')}`)
            if (flags[flag]) throw new Error(`Duplicate flag '${flag}': --${flag} was already set`) // flag dupe check
            ...
            const next_flag_offset = args.slice(i+1).findIndex(element => element.startsWith("-"))

            // slice to the end if none found, else slice to the next flag
            const next_flag_index = next_flag_offset === -1 ? args.length : i + 1 + next_flag_offset
            ...
            const flag_args = (preserve_case ? raw_args : args).slice(i+1, next_flag_index).join(" ")
    ...
    switch (cmd){
        case "create":
            ...
            if (flags["status"]){
                if (flags["status"] === "closed") throw new Error(`Invalid status 'closed': issues cannot be created with a closed status`)
            } else {
                flags["status"] = "open";
            }
            break
