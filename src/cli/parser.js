...
export function parse(argv) {
    ...
    if (args[0] !== 'mt')
        throw new Error(`Unknown prefix '${args[0]}': must start with mt`) // check if first arg is "mt"
    ...
    const in_between = raw_args
        .slice(2, first_flag === -1 ? args.length : first_flag)
    ...
    while (i < args.length) {
        ...
        if (current.startsWith('-')) {
            ...
            if (!/^--[a-z]/.test(current))
                throw new Error(
                    `Invalid flag '${current}': flags must start with --`
                )
            ...
            if (flags[flag])
                throw new Error(
                    `Duplicate flag '${flag}': --${flag} was already set`
                ) // flag dupe check
    ...
    switch (cmd) {
        case 'create':
            if (!flags['title'])
                throw new Error(`Missing required input: title`) // no empty titles
    ...
    if (expected_flag_counts[cmd]) {
        ...
        if (min && count < min)
            throw new Error(
                `Too few flags: '${cmd}' expects at least ${min} flag(s)`
            )
        if (max && count > max)
            throw new Error(
                `Too many flags: '${cmd}' expects at most ${max} flag(s)`
            )
    }
