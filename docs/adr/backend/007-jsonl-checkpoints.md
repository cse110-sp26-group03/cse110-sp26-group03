...
We should use Option 1: Store the byte offset in an SQLite cache.

Write to/change the checkpoint value in the cache when we append to the JSONL log. This way, we are constantly up-to-date.
